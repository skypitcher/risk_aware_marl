...
class PrimalAvgAgent:
    ...
    def _train_step(self) -> None:
        ...
        if self.use_single_cost_critic:
            ...
            cost_loss = weighted_mse_loss(qc.gather(-1, batch.actions), target_cost, batch.weights)
        else:
            qc, qc1, qc2 = self.Qc(batch.states)
            cost_loss = weighted_mse_loss(qc1.gather(-1, batch.actions), target_cost, batch.weights) + weighted_mse_loss(
                qc2.gather(-1, batch.actions), target_cost, batch.weights
            )
        ...
        if self.training_steps % self.actor_update_freq == 0:
            ...
            if self.training_steps >= self.update_lambda_after_step:
                ...
                if self.training_steps % (self.cost_multiplier_update_freq * self.actor_update_freq) == 0:
                    lambda_loss = weighted_mean(self.lambdar() * (batch.target_costs - expected_cost), batch.weights)
...
class PrimalAvg(BatchedRLAgent):
