...
`sat_net/` contains the fixed-step, data-oriented satellite routing simulator. `sat_net/network.py` stores topology, neighbor/link matrices, and dense SPF next-hop rows in NumPy arrays, `sat_net/sim_kernel.py` owns flowlet/link array transitions, `sat_net/routing_env.py` orchestrates topology, traffic, and solver calls, and `sat_net/solver/` defines the batched routing policy contract plus the SPF baseline. Root-level scripts are entry points: `run_train.py`, `run_eval.py`, `run_spf.py`, and plotting utilities named `plot_*.py`. JSON environment and solver settings live in `configs/`. Static map data is in `assets/`; generated figures and run outputs are in `figs/` and `runs_*`.
